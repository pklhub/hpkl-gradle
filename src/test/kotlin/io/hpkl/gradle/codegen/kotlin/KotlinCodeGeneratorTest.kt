...
class KotlinCodeGeneratorTest {
    ...
    fun `testDefaultValues`() {
      ...
      class TestClass {
        ...
        lb: List<Boolean> = List(true, false)
        lbytes: List<Int8> = List(1,2)
        ldurations: List<Duration> = List(1.min,2.min)
        map: Map<String, Int> = Map("1", 2)
        inner: Inner = new Inner { i = 1 }
      }
    """
                    .trimIndent(),
                mutableObjects = true,
        ...
        assertThat(kotlinCode)
            ...
            .contains(
                ...
                |    var bt: Byte = 1,
                |    var st: Short = 2,
                |    var d: Duration = Duration.parseIsoString("PT2M"),
                |    var l: List<Long> = listOf(1L,2L,3L),
                |    var lb: List<Boolean> = listOf(true,false),
                |    var lbytes: List<Byte> = listOf(1.toByte(),2.toByte()),
                |    var ldurations: List<Duration> =
                |        listOf(Duration.parseIsoString("PT1M"),Duration.parseIsoString("PT2M")),
